...
def parse_dotnet_test_results(output):
    ...
    # Typical summary line: "Failed:     1, Passed:    19, Skipped:     0"
...
def extract_failures_from_text(output, service_name, order_tested):
    """
    Find failure header lines and capture whole contiguous blocks for each failure.
    Returns a list of failure dicts (service, test_name, status, message, order_tested).
    """
    lines = output.splitlines()
    failures = []

    # Header regexes (cover multiple variations)
    header_regexes = [
        re.compile(r'^\s*Failed\s+(.+?)\s*\[', re.IGNORECASE),          # "Failed Namespace.Class.Method [95 ms]"
        re.compile(r'^\s*(.+?)\s+\[(?:FAIL|FAILED)\b', re.IGNORECASE),  # "Namespace.Class.Method [FAIL]"
...
def find_trx_file(search_dir, prefix=None):
    """
    Find the most recently modified .trx file under search_dir (recursive).
    If prefix is provided, prefer files containing that prefix.
    """
...
def parse_trx_for_failures(trx_path, service_name, order_tested):
    ...
    # iterate all elements; handle namespaces by matching tag suffix
    for elem in root.iter():
        if elem.tag.endswith('UnitTestResult'):
            ...
            if outcome and outcome.lower() == 'failed':
                ...
                # extract message/stacktrace if present under Output/ErrorInfo
                message_parts = []
                for sub in elem.iter():
                    # check child tags by suffix
                    tag = sub.tag.lower()
                    ...
                    if tag.endswith('stacktrace') and (sub.text and sub.text.strip()):
                        message_parts.append(sub.text.strip())
                    # Some TRX layouts put the message under Output/ErrorInfo/Message, so we capture that above
...
def extract_failures(output, service_name, order_tested, project_dir=None):
    """
    Try text parsing first; if no failures found but summary indicates failures
    (or we still have zero extracted), try parsing TRX files in the project_dir.
    """
    failures = extract_failures_from_text(output, service_name, order_tested)
    if failures:
        return failures

    # fallback: parse TRX files if provided
    if project_dir:
        trx = find_trx_file(project_dir)
        if trx:
            trx_failures = parse_trx_for_failures(trx, service_name, order_tested)
            if trx_failures:
                return trx_failures
...
def run_dotnet_tests(service_path, service_name, order_tested, failed_tests):
    """
    Run tests for a service. Creates a TRX file and test log, then extracts failures.
    Returns passed, failed, skipped, has_tests (bool).
    """
    has_trait = has_trait_annotation(service_path)
    test_project = None

    # find test csproj (common layout: a Tests folder containing csproj)
    for root, _, files in os.walk(service_path):
        for file in files:
    ...
    # Ensure we write log & TRX in the project folder
    ...
    if failed > 0:
        ...
        if not extracted:
            # Last-ditch: try parse TRX explicitly by searching project_dir
        ...
        if extracted:
            ...
            # if still nothing, add a minimal failure entry so the summary aligns with tests count
            failed_tests.append({
                "service": service_name.lower(),
                "test_name": "unknown",
                "status": "failed",
